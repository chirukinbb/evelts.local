...
@section('content')
    <form action="{{route('admin.events.store')}}" method="post" class="pt-3" enctype="multipart/form-data">
        ...
        <textarea rows="5" class="form-control mb-3 validation" name="description"
                  placeholder="Event legend">{{$event->description}}</textarea>
        <div class="row mb-3">
            <div class="col-6">
                <select name="user_id" class="col-6 form-control validation">
                    ...
                    @foreach($users as $user)
                        <option
                            value="{{$user->id}}" {{($event->user_id === $user->id ? 'selected' : '' )}}>{{$user->name}}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-6">
                <select name="category_id" class="col-6 form-control validation">
                    ...
                    @foreach($categories as $category)
                        <option
                            value="{{$category->id}}" {{($event->category_id === $category->id ? 'selected' : '' )}}>{{$category->title}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="row mb-3">
            ...
            <div class="col-4">
                <select id="timepicker" class="form-control">
                    @for($i = 0;$i < 24;$i ++)
                        <option
                            value="{{$i}}" {{(\Illuminate\Support\Carbon::parse($event->planing_time/1000)->hour === $i ? 'selected' : '' )}}>
                            {{$i > 9 ? $i : '0'.$i}}:00
                        </option>
                    @endfor
                </select>
